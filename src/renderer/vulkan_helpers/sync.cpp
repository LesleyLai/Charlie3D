    ...
    -> Expected<VkFence>
{
  ...
  if (set_debug_name(context, beyond::bit_cast<uint64_t>(fence), VK_OBJECT_TYPE_FENCE,
    ...
    -> Expected<VkSemaphore>
{
  ...
  if (set_debug_name(context, beyond::bit_cast<uint64_t>(semaphore), VK_OBJECT_TYPE_SEMAPHORE,
                     create_info.debug_name)) {
    report_fail_to_set_debug_name(create_info.debug_name);
  }
